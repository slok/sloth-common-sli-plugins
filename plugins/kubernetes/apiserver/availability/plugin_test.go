...
func TestSLIPlugin(t *testing.T) {
	tests := map[string]struct {
		...
		"Not having a filter shouldn't fail and return the correct query.": {
			expQuery: "\nsum(rate(apiserver_request_total{ code=~\"(5..|429)\" }[{{.window}}]))\n/\nsum(rate(apiserver_request_total{  }[{{.window}}]))\n",
		},

		"Having a filter should return the query with the filters.": {
			options:  map[string]string{"filter": `k1="v2",k2="v2"`},
			expQuery: "\nsum(rate(apiserver_request_total{ k1=\"v2\",k2=\"v2\",code=~\"(5..|429)\" }[{{.window}}]))\n/\nsum(rate(apiserver_request_total{ k1=\"v2\",k2=\"v2\" }[{{.window}}]))\n",
		},
